...
public class StagefourtestdocimplTest {
...
//
        ...
        public void TestSerialize(){
            ...
            store.putDocument(new ByteArrayInputStream(this.pdfData1), this.uri1, DocumentStore.DocumentFormat.PDF);
            store.putDocument(new ByteArrayInputStream(this.pdfData2), this.uri2, DocumentStore.DocumentFormat.PDF);
            List<String> strings = store.searchByPrefix("doc");
            assertEquals(2, strings.size());
            BTreeImpl<URI, Document> btree = store.getBTree();
            try {
                btree.moveToDisk(uri1);
            } catch (Exception e) {
                e.printStackTrace();
            }
            List<String> string = store.searchByPrefix("doc");
            assertEquals(2, string.size());
            Document doc1 = btree.get(uri1);
            store.deleteDirectory(uri1);
            store.deleteDirectory(uri2);
        }
//
        @Test
        public void memoryLimit(){
            ...
            assertTrue(store.getDocument(uri1) == null);
            store.deleteDirectory(uri1);
        }
        ...
        public void getDocTest(){
            ...
            store.putDocument(new ByteArrayInputStream(this.pdfData2), this.uri2, DocumentStore.DocumentFormat.PDF);
            store.putDocument(new ByteArrayInputStream(this.pdfData3), this.uri3, DocumentStore.DocumentFormat.PDF);
            store.setMaxDocumentBytes(pdfData1.length + pdfData2.length);
            store.setMaxDocumentCount(2);
            assertTrue(store.getDocument(uri1) == null);
            assertTrue(store.getDocument(uri2) != null);
            assertTrue(store.getDocument(uri3) != null);
            store.search("doc1");
            assertTrue(store.getDocument(uri1) != null);
            assertTrue(store.getDocument(uri2) == null);
            assertTrue(store.getDocument(uri3) != null);
